...
function Wizard:new(x, y)
    ...
    if not Wizard.image then
        -- Using character sprite as wizard placeholder
        local success, result = pcall(love.graphics.newImage, "assets/2D assets/Monochrome RPG Tileset/Dot Matrix/Sprites/character3.png")
        if success then
            Wizard.image = result
