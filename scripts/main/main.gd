...
func _ready() -> void:
	...
	if hud.has_method("bind_cooldown_system"):
		hud.bind_cooldown_system(cooldown_system, PRIMARY_COOLDOWN_KEY, MISSILES_COOLDOWN_KEY, ULTIMATE_COOLDOWN_KEY)
...
func _process(delta: float) -> void:
	...
	if _xp_system.has_pending_level_up():
		_upgrade_choices = UpgradeCatalog.get_choices(_xp_system.level)
